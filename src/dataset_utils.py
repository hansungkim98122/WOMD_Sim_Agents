...
def load_dataset(filepath: str, use_motion=True):
    ...
    training_dataset = tf.data.TFRecordDataset(tf.io.matching_files(TRAIN_FILES))
    validation_dataset = tf.data.TFRecordDataset(tf.io.matching_files(VALIDATION_FILES))
    test_dataset = tf.data.TFRecordDataset(tf.io.matching_files(TEST_FILES))

    return training_dataset, validation_dataset, test_dataset, features_description

def check_data_sample(dataset: tf.data.TFRecordDataset):
...
def visualize_scenario_sim_agents(dataset: tf.data.TFRecordDataset):
  dataset_iterator = dataset.as_numpy_iterator()
  bytes_example = next(dataset_iterator)
  scenario = scenario_pb2.Scenario.FromString(bytes_example)
  print(f'Checking type: {type(scenario)}')
